...
enum header_version {
	HEADER_PLAIN_BLAKE2b_16B, /** plaintext, keyless BLAKE2b
				     crypto_generichash() 16B */
...
struct header {
	union {
		struct {
			...
			uint8_t fec_percent; /**
					      * FEC percentage (multiplier in
					      * hundredths above 1.00):
					      * 'fec_percent * 0.01 + 1.0 ==
					      * fec_ratio' 1 == 1.01; 30 == 1.3;
					      * 101 == 2.01; 255 == 3.55;
					      */
			enum header_version version; /** verify protocol version
							compatibility */
		} __attribute__((packed));
...
enum message_stages {
	...
	MESSAGE_TX_KNOWN = 0x0002, /** at least one 'struct message' sent (seen
				      by network) */
	...
	MESSAGE_TX_DONE = 0x0080, /** all known receivers have received; can
				     dealloc */

	/* reserved 0x0100 */
	MESSAGE_RX_KNOWN = 0x0200, /** at least one 'struct message' received */
	MESSAGE_RX_DATA = 0x0400, /** all 'data' blocks decoded (or none
				     present) */
	MESSAGE_RX_META = 0x0800, /** all 'meta' blocks decoded (or none
				     present) */
...
/**
 * Leader (aka block header): first symbol in a block describes the block (esi
 * == 0)
 */
struct leader {
	struct header head;
	...
	uint32_t id_prev; /** block_id of previous block; self.head.block_id ==
			     beginning of sequence */
	uint32_t id_next; /** block_id of next block; self.head.block_id == end
			     of sequence */

	struct message message; /** all blocks belonging to a message carry the
				   same message */
...
typedef void (*message_status_t)(uint64_t message_id,
				 enum message_stages stages, void *context);

/** actively poll for status */
message_status_t message_status;

/* returns message_id */
uint64_t message_tx(struct channel *chan, const void *data, size_t data_len,
		    const void *meta, size_t meta_len,
		    message_status_t callback, void *context);

int message_listen(struct channel *chan, message_status_t callback,
		   void *context);

int message_rx(struct channel *chan, uint64_t message_id, void **data,
	       size_t *data_len, void **meta, size_t *meta_len);
